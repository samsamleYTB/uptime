...
class Database {
    ...
    static async patch(port = undefined, hostname = undefined) {
        // Still need to keep this for old versions of RavoHost
        ...
        try {
            ...
            if (e.message.includes("the following files are missing:")) {
                ...
                log.warn("db", "Database migration failed, you may be downgrading RavoHost.");
            } else {
    ...
    static async patchSqlite() {
        ...
        if (version === this.latestVersion) {
            ...
            try {
                ...
                log.error("db", ex);
                log.error("db", "Start RavoHost failed due to issue patching the database");
                log.error(
                    "db",
                    "Please contact support@ravohost.fr if you still encounter the problem after restart"
                );
