...
class Bark extends NotificationProvider {
    ...
    async send(notification, msg, monitorJSON = null, heartbeatJSON = null) {
        ...
        if (msg != null && heartbeatJSON != null && heartbeatJSON["status"] === UP) {
            let title = "RavoHost Monitor Up";
            return await this.postNotification(notification, title, msg, barkEndpoint);
        }

        if (msg != null && heartbeatJSON != null && heartbeatJSON["status"] === DOWN) {
            let title = "RavoHost Monitor Down";
            return await this.postNotification(notification, title, msg, barkEndpoint);
        }

        if (msg != null) {
            let title = "RavoHost Message";
            return await this.postNotification(notification, title, msg, barkEndpoint);
        }
    }
        ...
        if (notification.apiVersion === "v1" || notification.apiVersion == null) {
            ...
            result = await axios.post(
                endpoint,
                {
                    ...
                    group: notification.barkGroup || "RavoHost", // default group is RavoHost
                },
                config
            );
        }
