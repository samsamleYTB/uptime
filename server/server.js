/*
 * RavoHost Status Server
 * node "server/server.js"
 * DO NOT require("./server") in other modules, it likely creates circular dependency!
 */
console.log("Welcome to RavoHost Status");
...
// Exit RavoHost immediately if the Node.js version is banned
...
log.info("server", "RavoHost Status Version:", checkVersion.version);
...
(async () => {
    ...
    app.get("/.well-known/change-password", async (_, response) => {
        response.type("text/plain").send("To reset your password, run `npm run reset-password` on the server.");
    });
        ...
        socket.on("setup", async (username, password, callback) => {
            try {
                ...
                if ((await R.knex("user").count("id as count").first()).count !== 0) {
                    throw new Error(
                        "RavoHost has been initialized. If you want to run setup again, please delete the database."
                    );
                }
...
async function initDatabase(testMode = false) {
    ...
    // If there is no record in user table, it is a new RavoHost instance, need to setup
    if ((await R.knex("user").count("id as count").first()).count === 0) {
...
let unexpectedErrorHandler = (error, promise) => {
    ...
    console.error("If you keep encountering errors, please contact support@ravohost.fr");
};
